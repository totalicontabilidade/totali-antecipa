...
function mostrarErros(lista, subtitulo) {
  if (!lista || !lista.length) return;
  $('meSub').textContent = subtitulo || (lista.length + ' nota(s)');
  $('meLista').innerHTML = lista.map(e => `<tr><td><b>${esc(e.nNF || '—')}</b></td><td>${esc(e.emitente || '—')}${e.uf ? ' <span class="badge b-muted">' + esc(e.uf) + '</span>' : ''}</td><td class="mono" style="font-size:11px">${esc(e.chave || '—')}</td><td class="small">${esc(e.motivo || '')}</td></tr>`).join('');
